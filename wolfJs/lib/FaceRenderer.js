export class FaceRenderer {
  ...
  renderWalls() {
    ...
    let intersectRight = null;
    let intersectLast = null;
    let xLeft = 0;
    let xRight = 0;

    for (let i = 0; i < this.w; i++) {
      let dir = angleRight + i * deltaAngle;
      let intersect = this.getIntersection(x, y, dir);
      
      if(intersectLeft) {
        if(intersect.t.id !== intersectLeft.t.id || intersect.s !== intersectLeft.s) {
          intersectRight = intersectLast;
          xRight = i;
        } else if (i === this.w-1) {
          intersectRight = intersectLast;
          xRight = i;
      ...
      if(intersectRight) {
        ...
        let lLeft = this.getLineLength(intersectLeft.d);
        let lRight = this.getLineLength(intersectRight.d);

        this.ctx.fillStyle = intersectLeft.t.rgbString(intersectLeft.d);
        this.ctx.beginPath();

        this.ctx.moveTo(xLeft-1.5, this.h / 2 - lLeft / 2);
        this.ctx.lineTo(xLeft-1.5, this.h / 2 + lLeft / 2);
        this.ctx.lineTo(xRight+0.5, this.h / 2 + lRight / 2);
        this.ctx.lineTo(xRight+0.5, this.h / 2 - lRight / 2);
        this.ctx.closePath();
        this.ctx.fill();
        // draw face
